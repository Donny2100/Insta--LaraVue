...
// state-hidden
// city-hidden
if ($('#country-hidden').val()) {
    callAPI({
        url: getUrlAPI('/state/country/' + $('#country-hidden').val(), 'api'),
        method: "get",
    }).then(res => {
        if (res.code === 200) {
            $('#state_id').val(null);
            ...
            $('#state_id').val($('#state-hidden').val());

            callAPI({
                url: getUrlAPI('/cities/state/' + $('#state-hidden').val(), 'api'),
                method: "get",
            }).then(res => {
                $('#select_city').val(null);
                $.each($('#select_city option'), function(i, opt) {
                    opt.remove();
                });

                $('#select_city').append('<option value="">-- Select a state --</option>');

                res.data.forEach(function(state) {
                    $('#select_city').append('<option value="' + state.id + '">' + state.name + '</option>');
                });

                $('#select_city').val($('#city-hidden').val());
            });
        } else {
            console.log(res);
    ...
    if (!!country_id) {
        callAPI({
            ...
            if (res.code === 200) {
                ...
                $('#select_city').append('<option value="">-- Select a state first --</option>');
                $('#state_id').append('<option value="">-- Select a state --</option>');
...
$('#state_id').change(function () {
    ...
    callAPI({
        ...
        if (res.code === 200) {
            ...
            $('#select_city').append('<option value="">-- Select a city --</option>');
                    ...
                    let html = `
                                <div class="col-sm-2 media-thumb-wrap">
                                    <figure class="media-thumb">
                                        <img src="https://insta-help.s3.amazonaws.com/${res.file_name}">
