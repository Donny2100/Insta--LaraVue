(function ($) {
    ...
    $(document).on("click", ".city_edit", function () {
        ...
        let city_country_id = $(this).attr('data-country_id');
        let city_state_id = $(this).attr('data-state_id');
        ...
        callAPI({
            url: getUrlAPI('/state/country/' + city_country_id, 'api'),
            method: "get",
        }).then(res => {
            if (res.code === 200) {
                $('#state_id').val(null);
                $.each($('#state_id option'), function(i, opt) {
                    opt.remove();
                });
                ...
                if (findGetParameter('state_id')) {
                    $('#state_id').val(findGetParameter('state_id'));
                }
            } else {
                console.log(res);
                notify('It was not possible to fetch States!', 'error');
        ...
        if (city_thumb) {
            $('#preview_thumb').attr('src', city_thumb);
        } else {
            $('#preview_thumb').attr('src', `https://via.placeholder.com/120x150?text=insta-help`);
        }

        if (city_banner) {
            $('#preview_banner').attr('src', city_banner);
        } else {
            $('#preview_banner').attr('src', `https://via.placeholder.com/372x150?text=insta-help`);
        }
    ...
    $('#btn_add_city').click(function () {
        ...
        if (!!selected_country_id) {
            let data_resp = callAPI({
                url: getUrlAPI('/state/country/' + selected_country_id, 'api'),
                method: "get",
            });
    ...
    $(document).on("change", "#city__country_id", function () {
        let country_id = $('#city__country_id').val();
