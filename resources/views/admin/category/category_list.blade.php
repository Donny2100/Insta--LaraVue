                ...
                <div class="x_content table-responsive">
                    <table class="table table-striped table-bordered golo-datatable">
                        ...
                        @foreach($categories as $category)
                            <tr>
                                <td>{{$category->id}}</td>
                                <td>{{$category->name ? $category->name : $category->getTranslation()['name']}}</td>
                                @if($type === \App\Models\Category::TYPE_PLACE)
                                ...
                                <td>
                                    <button type="button" class="btn btn-warning btn-xs category_edit"
                                            data-id="{{$category->id}}"
                                            data-name="{{$category->name ? $category->name : $category->getTranslation()['name']}}"
                                            data-slug="{{$category->slug}}"
                                            data-priority="{{$category->priority}}"
                                            data-isfeature="{{$category->is_feature}}"
                                            data-featuretitle="{{$category->feature_title ? $category->feature_title : $category->getTranslation()['feature_title']}}"
                                            data-colorcode="{{$category->color_code}}"
                                            data-icon="{{s3($category->icon_map_marker)}}"
